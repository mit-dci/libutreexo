#ifndef UTREEXO_V1_ACCUMULATOR_H
#define UTREEXO_V1_ACCUMULATOR_H
...
public:
    ...
    V1Accumulator(uint64_t num_leaves);
    virtual ~V1Accumulator();
     ...
     */
    virtual bool Verify(const BatchProof<Hash>& proof, const std::vector<Hash>& target_hashes) = 0;
     ...
     */
    bool Prove(BatchProof<Hash>& proof, const std::vector<Hash>& target_hashes) const;

    /** Return the root hashes (roots of taller trees first) */
    void Roots(std::vector<Hash>& roots) const;

    bool ComparePositionMap(V1Accumulator& other) const;
...
protected:
    ...
    /*
     * Node represents a node in the accumulator forest. This is used to create
     * an abstraction on top of a accumulator implementation, because it might
     * not use a pointer based tree datastructure but the verification and
     * modification algorithms are quite nicely expressed using one.
     */
    class Node;
    ...
    std::vector<NodePtr<Node>> m_roots;
     ...
     */
    virtual NodePtr<Node> SwapSubTrees(uint64_t from, uint64_t to) = 0;
    ...
    virtual NodePtr<Node> MergeRoot(uint64_t parent_pos, Hash parent_hash) = 0;
    /* Allocate a new leaf and assign it the given hash */
    virtual NodePtr<Node> NewLeaf(const Leaf& leaf) = 0;
